...
const QStringList &geti18nCategoryNames()
{
    static const QStringList names = {'''
        ...
        content += "\n".join([localized_header] + [f"        i18nc(\"Emoji Category\", \"{categoryName.decode('utf-8')}\")," for categoryName in self.categoryNames] + [template_foot])

        with open("../emojicategory.cpp", "w") as f:
            f.write(content)

# Naive sanity check for running it under correct directory
if not os.path.exists("../emojimodel.cpp"):
