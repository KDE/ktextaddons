...
QDebug operator<<(QDebug d, const TextAutoGenerateTextMcpProtocolCore::McpProtocolResourceUpdatedNotification &t)
{
...
McpProtocolResourceUpdatedNotification McpProtocolResourceUpdatedNotification::fromJson(const QJsonObject &obj)
{
    ...
    if (const QJsonValue paramsValue = obj.value("params"_L1); paramsValue.isObject()) {
        prompt.setParams(McpProtocolResourceUpdatedNotificationParams::fromJson(paramsValue.toObject()));
    }
    return prompt;
}
