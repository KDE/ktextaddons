...
#include "textutilstexttohtmltest.h"

#include "texttohtml/textutilstexttohtml.h"
#include "texttohtml/textutilstexttohtml_p.h"

#include <QDebug>
#include <QTest>
#include <QUrl>

QTEST_MAIN(TextUtilsTextToHtmlTest)

Q_DECLARE_METATYPE(TextUtils::TextUtilsTextToHtml::Options)
...
void TextUtilsTextToHtmlTest::testGetEmailAddress()
{
    // empty input
    const QString emptyQString;
    TextUtils::TextUtilsTextToHtmlHelper ll1(emptyQString, 0);
    QVERIFY(ll1.getEmailAddress().isEmpty());

    // no '@' at scan position
    TextUtils::TextUtilsTextToHtmlHelper ll2(QStringLiteral("foo@bar.baz"), 0);
    QVERIFY(ll2.getEmailAddress().isEmpty());

    // '@' in local part
    TextUtils::TextUtilsTextToHtmlHelper ll3(QStringLiteral("foo@bar@bar.baz"), 7);
    QVERIFY(ll3.getEmailAddress().isEmpty());

    // empty local part
    TextUtils::TextUtilsTextToHtmlHelper ll4(QStringLiteral("@bar.baz"), 0);
    QVERIFY(ll4.getEmailAddress().isEmpty());
    TextUtils::TextUtilsTextToHtmlHelper ll5(QStringLiteral(".@bar.baz"), 1);
    QVERIFY(ll5.getEmailAddress().isEmpty());
    TextUtils::TextUtilsTextToHtmlHelper ll6(QStringLiteral(" @bar.baz"), 1);
    QVERIFY(ll6.getEmailAddress().isEmpty());
    TextUtils::TextUtilsTextToHtmlHelper ll7(QStringLiteral(".!#$%&'*+-/=?^_`{|}~@bar.baz"), qstrlen(".!#$%&'*+-/=?^_`{|}~"));
    QVERIFY(ll7.getEmailAddress().isEmpty());

    // allowed special chars in local part of address
    TextUtils::TextUtilsTextToHtmlHelper ll8(QStringLiteral("a.!#$%&'*+-/=?^_`{|}~@bar.baz"), qstrlen("a.!#$%&'*+-/=?^_`{|}~"));
    QCOMPARE(ll8.getEmailAddress(), QStringLiteral("a.!#$%&'*+-/=?^_`{|}~@bar.baz"));

    // '@' in domain part
    TextUtils::TextUtilsTextToHtmlHelper ll9(QStringLiteral("foo@bar@bar.baz"), 3);
    QVERIFY(ll9.getEmailAddress().isEmpty());

    // domain part without dot
    TextUtils::TextUtilsTextToHtmlHelper lla(QStringLiteral("foo@bar"), 3);
    QVERIFY(lla.getEmailAddress().isEmpty());
    TextUtils::TextUtilsTextToHtmlHelper llb(QStringLiteral("foo@bar."), 3);
    QVERIFY(llb.getEmailAddress().isEmpty());
    TextUtils::TextUtilsTextToHtmlHelper llc(QStringLiteral(".foo@bar"), 4);
    QVERIFY(llc.getEmailAddress().isEmpty());
    TextUtils::TextUtilsTextToHtmlHelper lld(QStringLiteral("foo@bar "), 3);
    QVERIFY(lld.getEmailAddress().isEmpty());
    TextUtils::TextUtilsTextToHtmlHelper lle(QStringLiteral(" foo@bar"), 4);
    QVERIFY(lle.getEmailAddress().isEmpty());
    TextUtils::TextUtilsTextToHtmlHelper llf(QStringLiteral("foo@bar-bar"), 3);
    QVERIFY(llf.getEmailAddress().isEmpty());

    // empty domain part
    TextUtils::TextUtilsTextToHtmlHelper llg(QStringLiteral("foo@"), 3);
    QVERIFY(llg.getEmailAddress().isEmpty());
    TextUtils::TextUtilsTextToHtmlHelper llh(QStringLiteral("foo@."), 3);
    QVERIFY(llh.getEmailAddress().isEmpty());
    TextUtils::TextUtilsTextToHtmlHelper lli(QStringLiteral("foo@-"), 3);
    QVERIFY(lli.getEmailAddress().isEmpty());

    // simple address
    TextUtils::TextUtilsTextToHtmlHelper llj(QStringLiteral("foo@bar.baz"), 3);
    QCOMPARE(llj.getEmailAddress(), QStringLiteral("foo@bar.baz"));
    TextUtils::TextUtilsTextToHtmlHelper llk(QStringLiteral("foo@bar.baz."), 3);
    QCOMPARE(llk.getEmailAddress(), QStringLiteral("foo@bar.baz"));
    TextUtils::TextUtilsTextToHtmlHelper lll(QStringLiteral(".foo@bar.baz"), 4);
    QCOMPARE(lll.getEmailAddress(), QStringLiteral("foo@bar.baz"));
    TextUtils::TextUtilsTextToHtmlHelper llm(QStringLiteral("foo@bar.baz-"), 3);
    QCOMPARE(llm.getEmailAddress(), QStringLiteral("foo@bar.baz"));
    TextUtils::TextUtilsTextToHtmlHelper lln(QStringLiteral("-foo@bar.baz"), 4);
    QCOMPARE(lln.getEmailAddress(), QStringLiteral("foo@bar.baz"));
    TextUtils::TextUtilsTextToHtmlHelper llo(QStringLiteral("foo@bar.baz "), 3);
    QCOMPARE(llo.getEmailAddress(), QStringLiteral("foo@bar.baz"));
    TextUtils::TextUtilsTextToHtmlHelper llp(QStringLiteral(" foo@bar.baz"), 4);
    QCOMPARE(llp.getEmailAddress(), QStringLiteral("foo@bar.baz"));
    TextUtils::TextUtilsTextToHtmlHelper llq(QStringLiteral("foo@bar-bar.baz"), 3);
    QCOMPARE(llq.getEmailAddress(), QStringLiteral("foo@bar-bar.baz"));
}

void TextUtilsTextToHtmlTest::testGetUrl()
{
...
void TextUtilsTextToHtmlTest::testGetUrl2(const QString &left, const QString &right)
{
    QStringList schemas;
    ...
    for (const QString &schema : std::as_const(schemas)) {
        for (QString url : std::as_const(urls)) {
            ...
            TextUtils::TextUtilsTextToHtmlHelper ll(test, left.length());
    ...
    {
        TextUtils::TextUtilsTextToHtmlHelper ll(url, 0, 10);
        QVERIFY(ll.getUrl().isEmpty()); // url too long
    }
    {
        TextUtils::TextUtilsTextToHtmlHelper ll(url, 0, url.length() - 1);
        QVERIFY(ll.getUrl().isEmpty()); // url too long
    }
    {
        TextUtils::TextUtilsTextToHtmlHelper ll(url, 0, url.length());
...
void TextUtilsTextToHtmlTest::testHtmlConvert_data()
{
    QTest::addColumn<QString>("plainText");
    QTest::addColumn<TextUtils::TextUtilsTextToHtml::Options>("flags");
    ...
    QTest::newRow("") << " foo" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces) << "&nbsp;foo";
    QTest::newRow("") << "  foo" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces) << "&nbsp;&nbsp;foo";
    QTest::newRow("") << "  foo  " << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces) << "&nbsp;&nbsp;foo&nbsp;&nbsp;";
    QTest::newRow("") << "  foo " << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces) << "&nbsp;&nbsp;foo&nbsp;";
    QTest::newRow("") << "bla bla bla bla bla" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
                      << "bla bla bla bla bla";
    QTest::newRow("") << "bla bla bla \n  bla bla bla " << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
                      << "bla bla bla&nbsp;<br />\n&nbsp;&nbsp;bla bla bla&nbsp;";
    QTest::newRow("") << "bla bla  bla" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces) << "bla bla&nbsp;&nbsp;bla";
    QTest::newRow("") << " bla bla \n bla bla a\n  bla bla " << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
                      << "&nbsp;bla bla&nbsp;<br />\n&nbsp;bla bla a<br />\n"
                         "&nbsp;&nbsp;bla bla&nbsp;";
    ...
    QTest::newRow("") << "Ce paragraphe _contient_ des mots ou des _groupes de mots_ à mettre en"
                         " forme…"
                      << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces | TextUtils::TextUtilsTextToHtml::HighlightText)
    ...
    QTest::newRow("punctation-bug") << "Ce texte *a l'air* de _fonctionner_, à condition"
                                    ...
                                    << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces
                                                                               | TextUtils::TextUtilsTextToHtml::HighlightText)
    ...
    QTest::newRow("bug504078") << "You should see a _<foo>_ on this line."
                               << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces
                                                                          | TextUtils::TextUtilsTextToHtml::HighlightText)
    ...
    QTest::newRow("") << "QUrl url(\"http://strange<hostname>/\");"
                      << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::ReplaceSmileys | TextUtils::TextUtilsTextToHtml::HighlightText)
    ...
    QTest::newRow("url-with-url")
        << "foo <https://www.kde.org/ <https://www.kde.org/>>" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
        << "foo &lt;<a href=\"https://www.kde.org/ \">https://www.kde.org/ </a>&lt;<a href=\"https://www.kde.org/\">https://www.kde.org/</a>&gt;&gt;";

    // Fix url exploit
    QTest::newRow("url-exec-html") << "https://\"><!--" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
                                   << "https://&quot;&gt;&lt;!--";

    QTest::newRow("url-exec-html-2") << "https://192.168.1.1:\"><!--" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
                                     << "https://192.168.1.1:&quot;&gt;&lt;!--";

    QTest::newRow("url-exec-html-3") << "https://<IP>:\"><!--" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
                                     << "https://&lt;IP&gt;:&quot;&gt;&lt;!--";

    QTest::newRow("url-exec-html-4") << "https://<IP>:/\"><!--" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
                                     << "https://&lt;IP&gt;:/&quot;&gt;&lt;!--";
    ...
    QTest::newRow("url-with-ref-in-[") << "https://www.kde.org[1]" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
    ...
    QTest::newRow("url ends with double-quote") << "file:///srv/http\""
                                                << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
                                                << "file:///srv/http&quot;";
    QTest::newRow("url in middle of string ends with double-quote")
        << "echo \"Server = file:///srv/http\" >> /etc/pacman.conf" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
    ...
    const auto opt = TextUtils::TextUtilsTextToHtml::PreserveSpaces | TextUtils::TextUtilsTextToHtml::ConvertPhoneNumbers;
    // tel: urls
    ...
    // xmpp bug 422291
    QTest::newRow("xmpp1") << "xmpp:username@server.tld" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces)
    ...
    // Test news: only
    QTest::newRow("news") << "news: " << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces) << "news:&nbsp;";

    QTest::newRow("ftp") << "ftp: " << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces) << "ftp:&nbsp;";
    QTest::newRow("mailto") << "mailto: " << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces) << "mailto:&nbsp;";
    QTest::newRow("empty") << "" << TextUtils::TextUtilsTextToHtml::Options(TextUtils::TextUtilsTextToHtml::PreserveSpaces) << "";
}

void TextUtilsTextToHtmlTest::testHtmlConvert()
{
    QFETCH(QString, plainText);
    QFETCH(TextUtils::TextUtilsTextToHtml::Options, flags);
    ...
    const QString actualHtml = TextUtils::TextUtilsTextToHtml::convertToHtml(plainText, flags);
    QCOMPARE(actualHtml, htmlText);
}

#define s(x) QStringLiteral(x)

void TextUtilsTextToHtmlTest::testEmoticons_data()
{
...
void TextUtilsTextToHtmlTest::testEmoticons()
{
    QFETCH(QString, input);
    QFETCH(QString, output);
    QCOMPARE(TextUtils::TextUtilsTextToHtml::convertToHtml(input, TextUtils::TextUtilsTextToHtml::ReplaceSmileys | TextUtils::TextUtilsTextToHtml::IgnoreUrls),
             output);
}

void TextUtilsTextToHtmlTest::testEmoticonsNoReplace_data()
{
    QTest::addColumn<QString>("input");
...
void TextUtilsTextToHtmlTest::testEmoticonsNoReplace()
{
    QFETCH(QString, input);
    QCOMPARE(TextUtils::TextUtilsTextToHtml::convertToHtml(input, TextUtils::TextUtilsTextToHtml::ReplaceSmileys | TextUtils::TextUtilsTextToHtml::IgnoreUrls),
             input);
...
#include "moc_textutilstexttohtmltest.cpp"
