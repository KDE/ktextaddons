...
{
    if (index.data(TextAutoGenerateChatModel::SenderRole).value<TextAutoGenerateMessage::Sender>() == TextAutoGenerateMessage::Sender::LLM) {
...
void TextAutogenerateListViewDelegate::drawDate(QPainter *painter, const QModelIndex &index, const QStyleOptionViewItem &option) const
{
    ...
    const QRect dateAreaRect(option.rect.x() + margin, option.rect.y(), option.rect.width() - 2 * margin, dateSize.height()); // the whole row
...
QSize TextAutogenerateListViewDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    const QSize size = {layout.decoRect.width(), layout.decoRect.height()};
    if (!size.isEmpty()) {
...
TextAutogenerateListViewDelegate::MessageLayout TextAutogenerateListViewDelegate::doLayout(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    layout.decoRect = QRect(indent,
                            ...
                            maxWidth,
                            textSize.height() + TextAutogenerateDelegateUtils::spacingText() * 3);

    layout.textRect = QRect(indent + TextAutogenerateDelegateUtils::marginText(),
                            layout.decoRect.top() + TextAutogenerateDelegateUtils::spacingText(),
                            maxWidth - TextAutogenerateDelegateUtils::marginText(),
                            layout.decoRect.height() + TextAutogenerateDelegateUtils::spacingText() * 3);
    return layout;
}
