...
bool TextAutoGenerateListViewDelegate::enableTextToSpeech() const
{
    return mEnableTextToSpeech;
}

void TextAutoGenerateListViewDelegate::setEnableTextToSpeech(bool newEnableTextToSpeech)
{
                                                        ...
                                                        const MessageLayout &layout) const
{
    ...
    if (layout.dateSize.isValid()) {
        ...
        */
        ...
        if (isMouseOver) {
            // TODO fix 4 or 3 elements

            if (!inProgress) {
                iconSizeAreaWidth = 4 * (buttonIconSize(option) + TextAutoGenerateDelegateUtils::iconSpacing());
            } else {
    ...
    if (isMouseOver) {
        ...
        if (mEnableTextToSpeech && layout.textToSpeechIconRect.isValid()) {
