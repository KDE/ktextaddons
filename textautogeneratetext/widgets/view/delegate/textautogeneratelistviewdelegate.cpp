...
TextAutoGenerateListViewDelegate::TextAutoGenerateListViewDelegate(TextAutoGenerateText::TextAutoGenerateManager *manager, QListView *view)
    ...
    , mForkIcon(QIcon::fromTheme(u"state-fork"_s))
{
...
void TextAutoGenerateListViewDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (layout.textRect.isValid()) {
        ...
        if (messageBackgroundColor.isValid() && messageBackgroundColor != QColor(Qt::transparent)) {
            ...
            const bool isUser = (message->sender() == TextAutoGenerateMessage::Sender::User);
            painter->setBrush(
                                                        ...
                                                        const MessageLayout &layout) const
{
    ...
    if (isMouseOver) {
        ...
        if (layout.infoIconRect.isValid()) {
...
TextAutoGenerateListViewDelegate::MessageLayout TextAutoGenerateListViewDelegate::doLayout(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (message && message->messageAttachments() && !message->messageAttachments()->isEmpty()) {
    ...
    if (isUser && !waitingAnswer && !mShowArchive && !mInProgress) {
        ...
        if (const bool inProgress = index.data(TextAutoGenerateMessagesModel::FinishedRole).toBool(); !inProgress) {
    ...
    if (!isUser) {
        if (!mInProgress && !waitingAnswer && !mShowArchive) {
            positionIcon -= iconSize + TextAutoGenerateDelegateUtils::iconSpacing();
    ...
    if (!waitingAnswer && mShowFork) {
        positionIcon -= iconSize + TextAutoGenerateDelegateUtils::iconSpacing();
        layout.forkIconRect = QRect(positionIcon, lineY, iconSize, iconSize);
    }
#if HAVE_KTEXTADDONS_TEXT_TO_SPEECH_SUPPORT
...
bool TextAutoGenerateListViewDelegate::helpEvent(QHelpEvent *helpEvent, QAbstractItemView *view, const QStyleOptionViewItem &option, const QModelIndex &index)
{
    ...
    if (helpEvent->type() == QEvent::ToolTip) {
        ...
        if (layout.forkIconRect.contains(helpEventPos)) {
            QToolTip::showText(helpEvent->globalPos(), i18nc("@info:tooltip", "Fork"), view);
            return true;
        }
...
{
    if (const TextAutoGenerateListViewDelegate::MessageLayout layout = doLayout(option, index); maybeStartDrag(event, layout.textRect, option, index)) {
        return true;
    }
    return false;
}
...
bool TextAutoGenerateListViewDelegate::maybeStartDrag(QMouseEvent *mouseEvent, QRect messageRect, const QStyleOptionViewItem &option, const QModelIndex &index)
{
    ...
    if (mTextSelection->hasSelection()) {
        ...
        if (const int charPos = doc->documentLayout()->hitTest(pos, Qt::FuzzyHit); charPos != -1 && mTextSelection->contains(index, charPos)) {
...
bool TextAutoGenerateListViewDelegate::handleMouseEvent(QMouseEvent *mouseEvent,
                                                        ...
                                                        const QModelIndex &index)
{
    if (!decoRect.contains(mouseEvent->pos())) {
        return false;
    }

    const QPoint pos = mouseEvent->pos() - textRect.topLeft();
    ...
    case QEvent::MouseButtonPress:
        ...
        if (const auto *doc = documentForIndex(index, textRect.width())) {
    ...
    case QEvent::MouseButtonRelease: {
        ...
        if (layout.editedIconRect.contains(mouseEvent->pos())) {
            ...
            return true;
        } else if (layout.forkIconRect.contains(mouseEvent->pos())) {
            Q_EMIT forkRequested(index);
            return true;
        } else if (layout.textToSpeechIconRect.contains(mouseEvent->pos())) {
            if (const bool textToSpeechInProgress = index.data(TextAutoGenerateMessagesModel::TextToSpeechInProgressRole).toBool(); textToSpeechInProgress) {
    ...
    const QList<TextAutoGenerateMessageWaitingAnswerAnimation::ScaleAndOpacity> &scaleAndOpacities)
{
    ...
    TextAutoGenerateDelegateUtils::IndexScaleAndOpacities back{.index = index, .scaleAndOpacities = scaleAndOpacities};
