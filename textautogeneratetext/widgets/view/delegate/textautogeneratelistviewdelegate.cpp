...
TextAutoGenerateListViewDelegate::MessageLayout TextAutoGenerateListViewDelegate::doLayout(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (message->messageAttachments() && !message->messageAttachments()->isEmpty()) {
        ...
        layout.decoRect.translate(-(attachmentsSize.width() - layout.decoRect.width()), 0);
        layout.decoRect.setHeight(layout.decoRect.height() + attachmentsSize.height() + attachments.count() * offsetSpacing);
        layout.decoRect.setWidth(layout.decoRect.height() + attachmentsSize.width());
        // TODO fix me
        // layout.decoRect.moveLeft(50);
        // layout.textRect.moveLeft(50 + TextAutoGenerateDelegateUtils::marginText());

        layout.textRect.moveLeft(option.rect.width() - maxWidth - attachmentsSize.width() + TextAutoGenerateDelegateUtils::marginText());
    }
