/*
  SPDX-FileCopyrightText: 2025-2026 Laurent Montel <montel@kde.org>

  SPDX-License-Identifier: GPL-2.0-or-later
*/
#pragma once

#include <QObject>

class TextAutoGenerateLocalMessagesDatabaseTest : public QObject
{
    Q_OBJECT
public:
    explicit TextAutoGenerateLocalMessagesDatabaseTest(QObject *parent = nullptr);
    ~TextAutoGenerateLocalMessagesDatabaseTest() override = default;

private Q_SLOTS:
    void shouldDefaultValues();
    void initTestCase();
    void shouldVerifyDbFileName();
    void shouldPersistFirstInsertForFreshConnection();
    void shouldStoreMessages();
    void shouldDeleteMessages();
    void shouldReturnNullIfDoesNotExist();
    void shouldDeleteDatabaseWhenConnectionWasNeverOpened();
    void shouldReturnLastMessageDateTime();
};
