    ...
    : TextAutoGenerateText::TextAutoGenerateReply{netReply, requestType, parent}
{
    connect(mReply, &QNetworkReply::finished, mReply, [this] {
        ...
        qCDebug(AUTOGENERATETEXT_LMSTUDIO_LOG) << "LMStudio response finished";
        if (mDownloadError) {
            return;
        }
    ...
    connect(mReply, &QNetworkReply::errorOccurred, mReply, [this](QNetworkReply::NetworkError e) {
        if (e == QNetworkReply::NetworkError::AuthenticationRequiredError) {
            ...
            // TODO rename signal with more generic name
            Q_EMIT needOllamaLogin();
            Q_EMIT finished();
            return;
        }
        Q_EMIT errorOccurred(e);
        qCDebug(AUTOGENERATETEXT_LMSTUDIO_LOG) << "LMStudio HTTP error:" << e;
    });
    ...
    case RequestTypes::StreamingChat:
        for (const auto &tok : mTokens) {
            ...
            if (messageObj.contains("tool_calls"_L1)) {
                ...
                const QList<TextAutoGenerateReply::ToolCallArgumentInfo> infos = parseToolCallsOpenAI(array);
...
QString LMStudioReply::generateModelInfo() const
{
    QString mardown;
...
TextAutoGenerateText::TextAutoGenerateReply::DownloadModelInfo LMStudioReply::parseDownLoadInfo(const QJsonDocument &doc) const
{
