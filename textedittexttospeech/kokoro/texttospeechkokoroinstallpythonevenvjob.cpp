...
{
    if (!canStart()) {
        qCWarning(KOKORO_TEXT_TO_SPEECH_LIB_LOG) << "Impossible to start TextToSpeechKokoroInstallPythonEvenvJob";
    ...
    if (QDir venvDir(venvPath); venvDir.exists() && !venvDir.removeRecursively()) {
        qCWarning(KOKORO_TEXT_TO_SPEECH_LIB_LOG) << "Impossible to remove existing venv:" << venvPath;
        Q_EMIT installFailed();
        deleteLater();
        return;
    }
    ...
    connect(process, &QProcess::finished, this, [this, process, venvPath](int exitCode, QProcess::ExitStatus exitStatus) {
        ...
        if (exitStatus != QProcess::NormalExit || exitCode != 0) {
            qCWarning(KOKORO_TEXT_TO_SPEECH_LIB_LOG) << "Unable to create venv" << venvPath << process->readAllStandardError();
            Q_EMIT installFailed();
        } else {
            Q_EMIT installDone();
        }
    ...
    connect(process, &QProcess::errorOccurred, this, [this, process](QProcess::ProcessError error) {
        ...
        if (error == QProcess::FailedToStart) {
            qCWarning(KOKORO_TEXT_TO_SPEECH_LIB_LOG) << "Unable to start" << process->program();
            process->deleteLater();
            Q_EMIT installFailed();
            deleteLater();
        }
